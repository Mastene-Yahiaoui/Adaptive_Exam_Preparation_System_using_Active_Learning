    ...
    for (student_id, quiz_id), group in frame.sort_values(group_columns + ["question_in_quiz", "raw_row_index"]).groupby(group_columns, sort=False):
        ...
        for _, row in group.iterrows():
            ...
            features = {
                ...
                "accuracy_so_far": (total_correct / total_answered) if total_answered else 0.5,
                "number_of_questions_answered_so_far": int(total_answered),
                "accuracy_easy_so_far": (difficulty_correct["easy"] / difficulty_counts["easy"]) if difficulty_counts["easy"] else 0.5,
                "accuracy_medium_so_far": (difficulty_correct["medium"] / difficulty_counts["medium"]) if difficulty_counts["medium"] else 0.5,
                "accuracy_hard_so_far": (difficulty_correct["hard"] / difficulty_counts["hard"]) if difficulty_counts["hard"] else 0.5,
